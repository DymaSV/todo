...
func add(text string) error {
	...
	if err != nil {
		fmt.Errorf("could not encode task: %v", err)
	}

	f, err := os.OpenFile(dbPath, os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0666)
	if err != nil {
		fmt.Errorf("could not open file %s: %v", dbPath, err)
	}
	...
	if err := f.Close(); err != nil {
		fmt.Errorf("could not close file %s: %v", dbPath, err)
	}
	return nil
}
...
func list() error {
	...
	for {
		if len(b) == 0 {
			...
			return fmt.Errorf("remaining odd %d bytes, what to do?", err)
		}
